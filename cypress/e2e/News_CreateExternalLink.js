...
const externalLink = "thisisnotarealwebsite.earth";
const language = "English";
let newsType = "External link";
let title = `${newsType} news ${currentDate}`;

describe(`News Creation - External link News`, () => {
    beforeEach(() => {
    ...
    it("News Creation - Sort News", () => {
        searchForm.clickResetButton();
        cy.resultTableIsSortedBy("Title"); //ascending
        cy.resultTableIsSortedBy("Title"); //descending
        cy.resultTableIsSortedBy("Created"); //ascending
        cy.resultTableIsSortedBy("Created"); //descending
        resultTable.verifyHeaderExisted("ID");
        resultTable.verifyHeaderExisted("Title");
        resultTable.verifyHeaderExisted("Status");
        resultTable.verifyHeaderExisted("Created");
        resultTable.verifyHeaderExisted("Language"); 
        resultTable.verifyHeaderExisted("Action"); 
    });

    it("PA-15696 - News Creation - Create External link News", () => {
        ...
        cy.inputValue("Link", externalLink);
    ...
    it("PA-15695 - News Creation - View External link News Details", () => {
        ...
        cy.verifyInputtedValue("Link", externalLink);
    });

    it("PA-15697 - News Creation - Edit/Update External link News", () => {
        searchForm.inputValue("Title", title);
        searchForm.clickSearchButton();
        ...
        cy.inputValue("Link", externalLink + " - Edited");
        cy.intercept("POST", "**plugin=SaveNews*").as('save');
        cy.clickButton("Save");
        cy.wait('@save').its("response.statusCode").should('eq', 200);
    ...
    it("PA-15694 - News Creation - Search The Edited External link News", () => {
    ...
    it("News Creation - Validate External link News Details", () => {
        // Search for the news and validate if its title has been changed
        searchForm.inputValue("Title", title + " - Edited");
        searchForm.clickSearchButton();
        resultTable.waitForResultTableLoaded();
        ...
        cy.verifyInputtedValue("Link", externalLink + " - Edited");
    });

    it("PA-15700 - News Creation - Update External link News Status", () => {
    ...
    it("PA-15698 - News Creation - Delete the External link News", () => {
